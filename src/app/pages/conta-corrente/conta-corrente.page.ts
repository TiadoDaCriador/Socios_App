...
import {
  ...
  chevronUpOutline, walletOutline,
} from 'ionicons/icons';
...
export class ContaCorrentePage implements OnInit, OnDestroy {
  ...
  constructor(private modalCtrl: ModalController) {
    addIcons({ searchOutline, filterOutline, chevronDownOutline, chevronUpOutline, walletOutline });
  }
  ...
  async carregarMovimentos() {
    ...
    try {
      ...
      // const resposta = await fetch('https://api.exemplo.com/socios/me/conta-corrente', {
      //   headers: { Authorization: `Bearer ${token}` }
      // });
      // const dados = await resposta.json();
      // this.movimentos = dados.map((m: any) => ({
      //   ...m,
      //   data: new Date(m.data),
      // }));

      // Dados de exemplo — remover quando a API estiver ligada
      this.movimentos = [
  ...
  get movimentosFiltrados(): Movimento[] {
    ...
    if (this.pesquisa.trim())
      lista = lista.filter(m => m.descricao.toLowerCase().includes(this.pesquisa.toLowerCase()));
    if (this.dataInicio)
      lista = lista.filter(m => m.data >= new Date(this.dataInicio));
    if (this.dataFim) {
